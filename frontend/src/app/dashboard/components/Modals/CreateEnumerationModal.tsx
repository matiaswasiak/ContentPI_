...
const CreateEnumerationModal: FC<iProps> = ({
  isOpen,
  label,
  onClose,
  options
  ...
  const handleSubmit = async (): Promise<void> => {
    let emptyValues: any = getEmptyValues(values, [
      'enumerationName',
      'identifier'
    ])
    ...
    if (emptyValues) {
      ...
      waitFor(2).then(async () => {
        ...
        if (dataCreateEnumeration.createEnumeration) {
          redirectTo(
            `/dashboard/${appId}/master/schema/enumeration/${values.identifier}`
          )
        }
      })
    }
      ...
      <div className={styles.modal}>
        <div>
          <label htmlFor="modelName">
            Enumeration Name{' '}
            {required.enumerationName && <Badge danger>Required</Badge>}
          </label>
          <Input
        ...
        <div className={styles.values}>
          <label htmlFor="values">
            Values{' '}
            {required.enumerationValues && <Badge danger>Required</Badge>}
          </label>
          <Tags
            label="Add new value"
            tags={[]}
